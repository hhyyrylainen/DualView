...
public class GUIRealtimeUpdateService : RealTimeUpdateServiceBase, IDisposable
{
    private readonly ILogger<GUIRealtimeUpdateService> logger;
    private readonly IGuiConfigurationService guiConfigurationService;
    private HubConnection? hubConnection;

    private bool closed;

    public GUIRealtimeUpdateService(ILogger<GUIRealtimeUpdateService> logger,
        IGuiConfigurationService guiConfigurationService)
    {
        this.logger = logger;
        this.guiConfigurationService = guiConfigurationService;
    }
    ...
    public override async Task StartConnectionAsync()
    {
        ...
        if (hubConnection == null)
        {
            hubConnection = new HubConnectionBuilder()
                .WithUrl(new Uri(guiConfigurationService.BackendUrl, "/hubs/realtime"))
    ...
    public void Dispose()
    {
        if (hubConnection != null!)
        {
            hubConnection.DisposeAsync().AsTask().Wait(TimeSpan.FromSeconds(20));
        }
    }
